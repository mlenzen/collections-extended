...
def test_empty():
    ...
    rm.empty(2, 3)
    rm.empty(2, 3)
    assert rm == RangeMap.from_iterable(((1, 2, "a"), (3, 4, "c"), (4, None, "d"),))
    rm.empty(3.5, 4.5)
    assert rm == RangeMap.from_iterable(((1, 2, "a"), (3, 3.5, "c"), (4.5, None, "d"),))
...
def test_eq():
    ...
    assert RangeMap({1: "a", 2: "b", 3: "c", 4: "d", 5: "e"}) == RangeMap(
        {1: "a", 2: "b", 3: "c", 4: "d", 5: "e"}
    )
    assert RangeMap(default_value="z") == RangeMap(default_value="z")
    assert RangeMap(
        {1: "a", 2: "b", 3: "c", 4: "d", 5: "e"}, default_value="z"
...
def test_get_range():
    ...
    assert rm.get_range(1, 3) == RangeMap.from_iterable(((1, 2, "a"), (2, 3, "b")))
    assert rm.get_range(1.5, 3) == RangeMap.from_iterable(((1.5, 2, "a"), (2, 3, "b")))
    print_underlying(rm.get_range(start=3))
    assert rm.get_range(start=3) == RangeMap({3: "c", 4: "d", 5: "e"})
    assert rm.get_range(stop=3) == RangeMap.from_iterable(
        ((None, 1, "z"), (1, 2, "a"), (2, 3, "b"))
    )
...
def test_len():
    rm = RangeMap.from_iterable(((1, 2, "a"), (4, 5, "d"), (5, None, "e"),))
    assert len(rm) == 3
...
def test_keys():
    ...
    rm = RangeMap.from_iterable(((1, 2, "a"), (4, 5, "d"),))
...
def test_start():
    ...
    rm = RangeMap.from_iterable(((1, 2, "a"), (4, 5, "b"),))
    assert rm.start == 1
    rm = RangeMap.from_iterable(((None, 2, "a"), (4, 5, "b"),))
...
def test_end():
    ...
    rm = RangeMap.from_iterable(((1, 2, "a"), (4, 5, "b"),))
    assert rm.end == 5
    rm = RangeMap.from_iterable(((1, 2, "a"), (4, None, "b"),))
